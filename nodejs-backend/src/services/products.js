...
export async function getImages() {
  const files = await fs.promises.readdir(path.resolve(process.cwd() + "/public/assets/products/"));
  const result = files.filter((item) => !/(^|\/)\.[^/.]/g.test(item)).map((f) => config.remote + "/assets/products/" + f);
  return result;
}
